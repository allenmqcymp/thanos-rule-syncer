...
result=1
trap 'kill $(jobs -p); exit $result' SIGINT SIGTERM EXIT
...
curl \
    ...
    --request POST \
    --data '{"audience": ["observatorium"], "client_id": "thanos-rule-syncer", 
    "client_secret": "secret", "grant_types": ["client_credentials"], 
...
token=$(curl \
    ...
    --data client_id=thanos-rule-syncer \
